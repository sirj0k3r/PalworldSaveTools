...
- **すべての Pals** を最大化 — パーティー内のすべての pals、すべての Palbox ページ、またはすべてのベース ワーカーのすべての統計 (IVs、ソウル、ランク、レベル) を最大化します。チート モードの上限を考慮します。
- **不正な Pals** を修正 — プレーヤーごとに不正なステータス、スキル、または特性を持つ pals を検出して制限します。
...
- **クリーンアップ** — 無効/変更されたアイテム、無効な pals および passives、無効な構造を削除します。不正な pals を修正 (合法的な最大値に制限)。対空砲塔をリセットします。 private chests のロックを解除します。すべての構造物を修復します。
...
2. **データを探索する**
   - タブ — **マップ**、**ツール**、**プレイヤー**、**ギルド**、**拠点**、**プレイヤーインベントリ**、**ベースインベントリ**、**Pal Editor**、**除外** – を使用して、セーブデータを探索します。
...
- ホストは `0001.sav` を使用します。
- 各クライアントは、`1234.sav`、`9876.sav` などの固有の通常の UID 保存を使用します。
- プレーヤー A は、`0001.sav` で進行中の古いホストです。
- プレーヤー B は既存のクライアントであり、新しいホストになります。
...
**前提条件:**
- プレイヤー B は、以前にプレイヤー A のワールドに参加し、キャラクターを作成している必要があります。
- プレーヤー B の通常のプレーヤー セーブは `Players` フォルダーに存在する必要があります。
- プレイヤー A とプレイヤー B は両方ともレベル 2 以上である必要があります。
- 変更を加える前に、ワールド保存フォルダー全体をバックアップしてください。
- 保存を変更する前に、サーバーをシャットダウンするか、Palworld を閉じてください。

---

### 1. プレーヤー B をホスト スロットに交換します

**ホスト保存の修正** を開き、次を選択します。
```
...
```
プレーヤー B がホスト スロットを占有します。プレーヤー A の元のホストの進行状況は、プレーヤー B の以前の通常の UID に保存されます。

---

### 2. プレーヤー B を新しいホストとしてワールドを開始します

プレイヤー B がワールドをホストして Palworld を開始します。プレイヤー B が正しいキャラクター、レベル、インベントリ、pals、ギルド、拠点、所有権を持っていることを確認します。
...
### 3. プレイヤー A をプレイヤー B の世界に参加させます

プレイヤー A は、現在プレイヤー B がホストしているワールドに参加します。Palworld は、プレイヤー A がホストではなくなったため、新しい通常の UID を割り当てる場合があります。
...
Palworld はプレイヤー A に新しいキャラクターの作成を依頼する場合があります (予想)。プレーヤー A の当初の進行状況はまだ `1234.sav` です。

プレイヤー A が一時キャラクターを作成した後:
```
...
### 4. レベルプレイヤー A の一時キャラクター

1. プレイヤー A に一時キャラクターで少なくとも **レベル 2** に達してもらいます。
2. プレイヤー A にサーバーから退出してもらいます。
3. サーバーを完全にシャットダウンします。
4. ワールドセーブフォルダーを再度バックアップします。

**ホスト セーブを修正**するには、選択した両方のキャラクターが少なくともレベル 2 である必要があるため、レベル 2 が必要です。

---

### 5. プレイヤー A の元の進行状況を復元する

**ホスト保存の修正** を再度開き、次を選択します。
```
Source Player: Player A's original progress, 1234.sav
Target Player: Player A's new client UID, 3456.sav
```
移行を実行します。これはもう 1 つの双方向スワップであるため、次のようになります。
...
```
プレーヤー A の新しいクライアント UID は、プレーヤー A の元のキャラクターと進行状況を指すようになりました。
...
```
- プレイヤー B は、プレイヤー B のオリジナル キャラクターを使用してホストします。
- プレイヤー A は、プレイヤー A の復元されたオリジナル キャラクターを使用して参加します。
